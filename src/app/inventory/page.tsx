...
async function loadItemsForSkus(skusInput: string[]): Promise<ItemRow[]> {
  ...
  try {
    const { data, error } = await supabase
      ...
      .in('sku', variants);
    if (error) {
      console.error('[supabase.in(items)] error:', error);
    }
    fetched = data ?? [];
  } catch (e) {
    console.error('[supabase.in(items) exception]', e);
  }
  ...
  // 2) ILIKE fallback per missing (now with wildcards)
    ...
    for (const t of tries) {
      try {
        const { data, error } = await supabase
          ...
          .ilike('sku', `%${t}%`)
          .limit(1);
        if (error) console.error('[supabase.ilike] error:', { pattern: `%${t}%`, error });

        if (data && data[0]) { hit = data[0]; break; }
      } catch (e) {
        console.error('[supabase.ilike exception]', e);
      }
    }
      ...
      for (const t of tries) {
        try {
          ...
          const { data, error } = await supabase
            ...
            .limit(3);
          if (error) console.error('[supabase.or] error:', { t, tNoHyphen, error });
          ...
          console.error('[supabase.or exception]', e);
        }
      }
    }
  ...
  if (uomIds.length > 0) {
    try {
      const { data: uoms, error: uErr } = await supabase
        ...
        .select('id, code')
        .in('id', uomIds);
      if (uErr) console.error('[supabase.uom] error:', uErr);
      (uoms ?? []).forEach((u: any) => uomMap.set(u.id, u.code));
    } catch (e) {
      console.error('[supabase.uom exception]', e);
    }
  }
  ...
  const addLineBySku = async () => {
    ...
    try {
      ...
      const rowToUse: ItemRow = found && found.sku ? found : {
        sku, name: '(Unknown item)', uom_code: '-', purchase_price: null, gst_percent: null, margin_percent: null,
