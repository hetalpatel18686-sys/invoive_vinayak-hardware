  ...
  useEffect(() => {
    (async () => {
      const r = await supabase
        .from('invoices')
        .select('id, invoice_no, status, issued_at, grand_total')
    ...
    <Protected>
      <div className="card">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <div className="card">
            <div className="text-sm text-gray-600">Grand Total (last 100 invoices)</div>
        ...
        <tbody>
          {invoices.map((i) => (
            <tr key={i.id}>
              ...
              <td>{i.status}</td>
              <td>{i.issued_at}</td>
              <td>₹{Number(i.grand_total || 0).toFixed(2)}</td>
            </tr>
          ))}
        </tbody>
        </table>
